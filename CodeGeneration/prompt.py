...
class ChatPromptBase:
    ...
    def add_example(self, param_list, result_list):
        ...
        for param,result in zip(param_list, result_list):
            example[0] = {'role': 'user', 'content': self.user_prompt_template.invoke(param).text}
...
code_gen_instruct = ChatPromptBase(
    ...
    """,
    user = """这是我的需求，请根据它实现功能。
    @REQUIREMENT:{requirement}，该函数在遵守CODE_RULES的同时满足我的需求。
    """,
    user_input_var=["requirement"]
)
...
code_gen_retlist = ChatPromptBase(
    ...
    """,
    user="""这是我提供给你的多个需求编号和内容，请根据它们一次性生成所有函数的代码。
    @req_list:{req_list}
