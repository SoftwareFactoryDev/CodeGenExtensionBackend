...
from function.CodeBaseBuild.prompt import function_sum_template
...
def process_single_row(args):
    ...
    index, row, codebase_path, host, model, key = args
...
async def gen_function_sum_multy(
    asset_path,
    max_workers=4,
    host="http://10.13.1.102:8021/v1",
    model="deepseek-ai/DeepSeek-R1",
...
def sum_embedding(asset_path, url):
    ...
    codebase["sum_embedding"] = codebase["summary"].apply(
        lambda x: nlp_emb_api(x, url=url)
    )
    codebase.to_csv(asset_path, index=False, encoding="utf-8")
    return f"成功生成功能描述向量"
...
async def code_sum_tokenize_multy(asset_path, stopword_path=None, max_workers=4):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        codebase["sum_tokenize"] = list(
            executor.map(lambda x: process_summary(x, stopwords), codebase["summary"])
        )
...
def process_single_module(args):
    module, asset_list, host, model, key = args
    prompt_template = get_prompt_template()

    mask = asset_list["module"].str.contains(module["name"], na=False)
    match_assets = asset_list[mask]
    asset_list = match_assets[["name", "file_path", "signature", "summary"]].copy()
    asset_info = asset_in_module(asset_list)
...
async def gen_module_sum_multy(
    asset_path,
    info_path,
    max_workers=4,
    host="http://10.13.1.102:8021/v1",
    ...
    # 准备任务列表
    tasks = [
        (module, asset_list, deepcopy(host), deepcopy(model), deepcopy(key))
        for module in module_list
    ]
