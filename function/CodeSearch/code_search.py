...
from function.CodeSearch.util import check_repo_json
...
class NlRetriever:
    ...
    def cosine_distances(self, target_vector, vector_array):
        ...
        if isinstance(target_vector, np.ndarray) and target_vector.ndim == 2:
            similarities = cosine_similarity(target_vector, vector_array)
    ...
    def nlp_consine_compute(self, key_word_embedding, doc_embeddings) -> float:
        cos = self.cosine_distances(key_word_embedding, doc_embeddings)
        return cos
    ...
    def custom_retrieval(self, query_texts, emb_url, top_k=3):
        ...
        key_words_embedding = nlp_emb_api(query_texts, emb_url)
        end_time = time.time()
        ...
        repo_rank = zip(
            self.info_data,
            self.nlp_consine_compute(key_words_embedding, repo_embs),
        )
        end_time = time.time()
        ...
        module_rank = zip(
            module_infos, self.nlp_consine_compute(key_words_embedding, module_embs)
        )
        end_time = time.time()
