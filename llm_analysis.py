...
def analysis(requirement):
    ...
    prompt_templete = code_gen_instruct
    prompt_templete.generate_prompt(user_param={'history':history, 'requirement': requirement})
    ...
    # reference_retrievel
    k = config['CodeSearch']['topk']
    examples = code_search_custom(retriever=retriever, key_words=requirement, top_K=k, codebase_path=codebase_path, columns = config['CodeSearch']['columns'])
    param_list = []
    result_list = []
    ret_info = ''
    messages = prompt_templete.generate_message()
...
#     result = []
#     for index in range(0, len(req_list), 1):
#         print(f"Processing {index}: Index:{index}/{len(req_list)}")
#         req_group = req_list[index]
#         req_cont = req_group['Content']
       
#         response = analysis(req_cont)
#         result.append({
#             'requirement': req_cont,
#             'response': response,
#         })
#         print(f"""
...
#     # 只有在result不为空时才保存文件
#     if result:
#         with open(f'./result/result-{1}.json', 'w', encoding='utf-8') as f:
#             json.dump(result, f, ensure_ascii=False, indent=2)
#         with open(f'./result/result-{1}.txt', 'w', encoding='utf-8') as f:
#             for index, item in enumerate(result):
#                 f.write(f'【{index+1}/{len(req_list)}】{item["req_list"]}\n{item["response"]}\n\n')